# My-Portfolio
# Hi, I'm Nadia Gul 👋

### Research • Data Analysis • Machine Learning • Technical Writing

I'm an **MS Computer Science graduate from FAST-NUCES** with experience in research, data analysis, machine learning, technical writing, and academic documentation.

My work focuses on applying data-driven and AI-based approaches to real-world problems, with research interests in **Machine Learning, Data Mining, Federated Learning, Explainable AI, and research methodology**.

## 🔬 Research & Academic Work

* **Systematic Literature Review — AI Clinical Trial Matching**

  * Reviewed research across IEEE, PubMed, Springer, and Scopus.
  * Analyzed NLP, machine learning, and hybrid approaches.
  * Studied datasets including MIMIC-III, N2C2, and ClinicalTrials.gov.
  * Identified research gaps and future research directions.

* **Resource-Aware Federated Learning for IoT**

  * Master's thesis focused on efficient client selection and lightweight models.
  * Studying non-IID data, communication cost, convergence, and fairness.

## 📊 Data Analysis & Visualization

* Python
* Excel
* Tableau
* Scikit-learn
* Data preprocessing
* Feature engineering
* Exploratory data analysis
* Data visualization

## 🤖 Machine Learning & AI

* Machine Learning
* Deep Learning
* Transformers
* Computer Vision
* Explainable AI
* Federated Learning
* PyTorch
* Scikit-learn

## 📝 Research & Technical Writing

* Systematic Literature Reviews
* Research Reports
* Technical Documentation
* Data-driven Analysis
* LaTeX
* Microsoft Word
* PowerPoint

## 💼 Experience

**Artificial Intelligence Intern — Genesys Research Lab**

* Developed an explainable image-captioning system using BLIP.
* Worked with data preparation, model fine-tuning, evaluation, documentation, and deployment.

**Teaching Assistant — Data Visualization | FAST-NUCES**

* Evaluated assignments and projects using structured rubrics.
* Designed coursework and provided technical feedback.
* Communicated data visualization concepts clearly to students.

**Web Developer — Vantage Droid Private Limited**

* Developed authentication and user-management systems.
* Built SQL-based administrative functionality and responsive web interfaces.

## 🏆 Certifications

* Prompt Engineering for ChatGPT — Vanderbilt University, Coursera
* AWS Academy Graduate — Cloud Security Foundations
* AWS Academy Graduate — Cloud Security Builder
* Introduction to AWS Cloud — Builder Labs
* Programming for Everybody — University of Michigan, Coursera
* Write Professional Emails in English — Georgia Institute of Technology
* The Complete Web Development Bootcamp — Udemy

## 📌 Featured Projects

* Explainable Image Captioning with BLIP
* Resource-Aware Federated Learning
* AI Clinical Trial Matching — Systematic Literature Review
* Speaker-Invariant Accent Classification
* Breast Cancer Prediction
* Heart Disease Prediction
* FIFA Interactive Data Visualization

## 📫 Connect With Me

* **LinkedIn:** [Nadia Gul](https://www.linkedin.com/in/nadia-gull-7ba818323/)
* **Email:** [nadiagul230200@gmail.com](mailto:nadiagul230200@gmail.com)

---

⭐ Feel free to explore my repositories and research projects.

